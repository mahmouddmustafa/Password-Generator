...
used_passwords = open("used_passwords.txt", "w")
used_passwords2 = open("used_passwords.txt", "r")
import random
import string
...
e = f'Your email is: {email}'
print(e)

print('Please ensure this password has not been used across other programs before.')
while True:
    password = input('Create a Password or type "random" to generate a password: ').strip()
    used_passwords.write(password + "\n")
    if password == "random":
...
while True:
    ...
    email_verify = input('re-enter your email: ').strip()
