...
while True:                                                                                                         #all conditions within the true will occur until the truth value is false
    ...
    if password in used_pass and password != "random":                                                              #checks if the password has been used before in the file and ensures the word "random"
       print('ERROR - This password has been used before, please enter a new one.')                                 #^continuation is not added to the used passwords file 
        ...
        special = lower = numbers = upper = False                                                   #set false until 'proven' true
        if password == "random":
                ...
                break
        for i in password:
            if not i.isalnum():                         #if the string is not alphaneumeric so not numbers or letters i.e. special characters/symbols - special is true
                special = True                              
            elif i.islower():                           #if the password has strings that are lower case - lower is true
                lower = True
            elif i.isdigit():                          #if the passord has strings that are digits/numbers - numbers is true
                numbers = True
            elif i.isupper():                           #if the password has strings that are uppercase - upper is true
                upper = True
            if special and lower and numbers and upper:     #if all of conditions are true, the for loop is stopped
                break
